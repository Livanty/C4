...
class GameScene: SKScene, SKPhysicsContactDelegate {
                ...
                case "jumpButton":
                    ...
                    if isOnGround {
                        ...
                        if let sparkle = SKEmitterNode(fileNamed: "JumpSpark") {
                            sparkle.name = "jumpSpark"
                            sparkle.position = CGPoint(x: 0, y: -character.size.height / 2)
                            sparkle.zPosition = 1
                            sparkle.targetNode = self  // Agar efek tetap muncul di scene
                            
                            character.addChild(sparkle)  // Menempel di karakter
                        }
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (maskA == 1 && maskB == 2) || (maskA == 2 && maskB == 1) {
            ...
            // Hapus spark dari karakter jika ada
            if let spark = character.childNode(withName: "jumpSpark") {
